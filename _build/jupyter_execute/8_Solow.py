...
# If you come here without expecting Japanese, please click [Google translated version](https://translate.google.com/translate?sl=auto&tl=en&u=https://py4macro.github.io/8_Solow.html) in English or the language of your choice.
...
# ここではモデルの具体的な説明については教科書に譲るとして，簡単にモデルの考え方を紹介し，重要な式をまとめることにする。
# 
# ＜記号＞
# * 産出量：$Y_t$
# * 消費量：$C_t$
...
# * 労働：$L_t$
# * 貯蓄率（一定な外生変数）：$0<s<1$
# * 労働人口増加率：$n\equiv\dfrac{L_{t+1}}{L_t}-1\geq 0$
# * 資本減耗率（一定な外生変数）：$0<d<1$
# * 生産性（一定な外生変数）：$A>0$
# 
# ＜一人当たりの変数＞
...
# 全ての市場は完全競争である閉鎖経済を想像しよう。この経済には一種類の財（ニューメレール財）しかなく，消費・貯蓄・投資に使われる。財は次の生産関数に従って生産される。
# 
# $$
...
# $$ (eq:8-production)
# 
# 消費者は所得の割合$s$を貯蓄するが，このモデルの中で消費者の役割はこれだけであり，残りは全ては生産側で決定される。貯蓄は$sY_t$であり投資$I_t$と等しくなる。
# 
# $$
# sY_t=I_t
# $$ (eq:8-syi)
...
# $$ (eq:8-kdot)
# 
# この式は非線形の差分方程式だが，全章でも述べたように，考え方は線形差分方程式と同じであり，数値計算のための`Python`コードに関しては大きな違いはない。また式[](eq:8-kdot)を資本ストックの成長率を示す式に書き換えることもできる。
# 
# $$
...
# $$ (eq:8-kss)
# 
# この値を生産関数[](eq:8-production)に代入することによりGDPの定常値を求めることができる。
# 
# $$
# y_*=Ak_*^{a}
# $$ (eq:8-yss)
...
# ````{admonition} コードの説明
# 1. `range(100,300,10)`は100から300-1-299までの間の数を10間隔で取得する。100，110，120,,,290となる。
# 1. `.subplots()`を使ってキャンバスを`fig`に，「軸」を`ax`に割り当てる。
# 1. `initial_list`に対して`for`ループを行う。一回のループごとに以下をおこなう。
#     * `i`は`k0`の引数に使う値になる。
#     * `solow_model()`で資本ストックの数値を計算し，その`DataFrame`を`df_temp`に割り当てる。
#     * `.plot()`を使い`ax`に図をプロットする。これにより図が重ね書きされる。
...
# {numref}`fig:8-solow`は定常状態は安定的であることを示している。またシミュレーションの結果も図から確認できる安定性と整合性がある結果である。次に，線形近似を使って解析的に安定性を確認してみることにする。また線形近似は真の値から乖離が発生するが，その乖離がどの程度のものかをコードを使って計算することにする。
# 
# ＜テイラー展開による１次線形近似＞
# * 関数$z=f(x)$を$x^*$でテイラー展開すると次式となる。
# 
#     $$
...
# ３行目は定常状態での式[](eq:8-kdot)と式[](eq:8-kss)を使っている。式[](eq:8-kapprox)は$k_{t+1}$と$k_t$の線形差分方程式になっており，$k_t$の係数は
# 
# $$
# 0<1-\lambda<1
# \quad\because
...
# 
# 式[](eq:8-lambda)は$k_t$の係数だが，その裏にあるメカニズムを考えてみよう。特に，$a$の役割を考える。$a$は資本の所得比率であり，目安の値は1/3である。そしてソロー・モデルにおける重要な役割が**資本の限界生産性の逓減**を決めるパラメータである。この効果により資本ストックが増加する毎に産出量も増加するがその増加自体が減少する。この効果により，{numref}`fig:8-solow`の曲線は凹関数になっており，生産関数[](eq:8-production)の場合は必ず45度線と交差することになる。即ち，資本の限界生産性の逓減こそが$k_t$が一定になる定常状態に経済が収束す理由なのである。この点がソロー・モデルの一番重要なメカニズムとなる。
# 
# ここで$a$が上昇したとしよう。そうなると資本の限界生産性の逓減の効果は弱くなり，資本ストックが増加しても産出量の増加自体の減少は小さくなる。また式[](eq:8-kss)が示すように定常状態での資本ストックより大きくなる。これは{numref}`fig:8-solow`で曲線が上方シフトしている考えると良いだろう。次章で収束速度について再考する際，$a$の大きさが重要になってくるので，この点を覚えておこう。

# ```{admonition} 内生的成長
# :class: note
# 更に$a$を上昇させて$a=1$になるとどうなるのだろう。この場合，資本の限界生産性は逓減せず一定となる。そして$k_t$の係数である式[](eq:8-coef_of_kt)は1になってしまい，$k_t$が一定になる定常状態が存在しなくなるのである。$a=1$の極限の状態を内生的成長と呼ぶ。ここでは立ち入った議論はしないが，内生的成長の典型的な生産関数は次式となり，
# 
# $$y_t=Ak_t$$
# 
# この生産関数に基づくモデルは$AK$モデルと呼ばれる。資本の限界生産性は$A$で一定となる。
# ```
...
# 初期の資本ストック$k_0$は同じなので誤差はゼロであり，$t=1$から線形近似の誤差が現れることになる。誤差は単調ではない。このことは{numref}`fig:8-solow`の図が示しているように，階段のような形で増加していくためである。ループの早い段階では線形近似の値は大き過ぎるため負の値になっているが、誤差は約0.02％であり、定常状態に近づくにつれて誤差はゼロに近づいている。もちろん，誤差の値は初期値が定常値から離れればそれだけ大きくなっていく。

# ## 長期均衡の予測

# ### 説明

# この節では長期均衡（定常状態）に焦点を当て，理論的な予測のデータとの整合性をチェックする。まず定常状態の特徴をまとめよう。式[](eq:8-kss)と[](eq:8-yss)を使いうと定常状態での一人当たりGDPは次式で与えれる。
# 
# $$
# k_*=
# \left(
...
# 
# この３つの予測が成立するか確かめに`py4macro`モジュールに含まれるPenn World Dataの次の変数を使う。
# * `cgdpo`：GDP（2019年）
# * `cn`：物的資本ストック（2019年）
# * `csh_i`：GDPに対しての資本形成の比率
#     * 投資の対GDPであり$s$の代わりに使う。
#     * 1960年〜2019年の平均を使う。
# * `emp`：雇用者数
#     * 1960年〜2019年の平均成長率を計算する。
# * `delta`：資本ストックの年平均減耗率
#     * 1960年〜2019年の平均を使う。
...
# 1960年から2019年までの国別の貯蓄率の平均を計算するが必要である。ここでは`DataFrame`のメソッド`.pivot()`を使いデータを整形する。`.pivot()`は，元の`DataFrame`から列を選び，その列から新たな`DataFrame`を作成する。実際にコードを実行して説明する。
...
# * `index`：`saving`の行ラベルを指定する。
# * `columns`：`saving`の列ラベルを指定する。
# * `values`：`saving`の値を指定する。
# ```
...
# ```{admonition} コードの説明
# `.dropna()`の引数は`axis='rows'`であり，引数を指定しない場合は`NaN`がある行が削除される。上のコードでは`axis='columns'`が指定されているため，`NaN`がある列が削除されている。
# ```
...
# ````{admonition} コードの説明
# 1. `.mean()`はそれぞれの列の平均を計算し，`Series`を返す。後で`DataFrame`を結合するために`.to_frame()`を使って`Series`を`DataFrame`に変換しており，引数`saving_rate`は列ラベルを指定している。もちろん引数を使わずに２行に分けることも可能である。
# ```
...
# 平均成長率を計算するには1960年と2019年の労働人口だけで計算できるが，同じ方法で計算してみる。
...
# 上で作成した`DataFrame`を結合する必要があり，そのための`DataFrame`のメソッド`.merge()`を説明する。`df_left`と`df_right`の二つの`DataFrame`があるとしよう。`df_left`を左の`DataFrame`，`df_right`を右の`DataFrame`と呼ぶことにする。`df_left`のメソッド`.merge()`を使い`df_right`と結合する場合，次のコードとなる。
# ```
...
# ```
# `left_on`は基準列に使う左の`DataFrame`にある列ラベルを文字列で指定する。同様に`right_on`は基準列に使う左の`DataFrame`にある列ラベルを文字列で指定する。デフォルトは両方とも`None`となっている。
# 
# ２つ目の問題は`how`という引数を使うことにより対処できる。使える値は次の４つであり，いずれも文字列で指定する。
# * `inner`：`df_left`と`df_right`の両方の基準列にある行だけを残す（デフォルト）。
...
# * `outer`：`df_left`と`df_right`の両方の行を残し，マッチする行がない場合は`NaN`を入れる。
# 
...
# AlbaniaやAngolaは`saving`，`depreciation`，`emp_growth`の`DataFram`には無いため`NaN`が入っている。
...
# * トレンド線の傾きの統計的優位性の表示
# 
# `for`ループを使ってこれらを同時に計算・表示しみる。まず`ky_ratio`の列名をみると，回帰分析の説明変数に使う変数が最後の３つに並んでいる。
...
# 5. `res`の属性`.params`は推定値を２つ返すが，1番目に傾きの推定値が格納されているので`[1]`で抽出し，`bhat`に割り当てている。
# 6. `res`の属性`.pvalues`は$p$値を２つ返すが，1番目に傾きの$p$値が格納されているので`[1]`で抽出し，`pval`に割り当てている。
# 7. `res`の属性`fittedvalues`は予測値を返すが，それを新たな列（ラベルは`Trend`）として`df_temp`に追加している。
# 8. `.subplots()`によるキャンバスを`fig`に，「軸」を`ax`に割り当てている。引数の`tight_layout=True`はスペースに余裕がある表示にするためであり，なくても構わない。
...
# ３つの図からソロー・モデルの理論的予測はデータと整合性があることが確認できる。ここで注意する点が一つある。式[](eq:8-kyratio)は因果関係を予測している。例えば，過去の貯蓄率が高くなることにより長期的な一人当たりGDPは増加する。一方，トレンド線は因果関係を示しているのではなく単なる相関関係を表している。ソロー・モデルの因果関係を計量経済学的に検討するにはさまざまな要因の検討が必要になり，本章の域を超える事になる。
