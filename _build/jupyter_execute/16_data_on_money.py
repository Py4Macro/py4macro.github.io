...
# * 1973年2月14日に変動相場制に移行
# 
# この政策変更は名目為替レートに関しての政策であり，「実質的」な要因の変化ではない。純粋に貨幣的な政策変更である。この金融政策変更の効果を考えるために，全ての財・サービスの価格は十分に伸縮的であり，瞬時に価格は調整され市場は常に精算する（需要と供給は常に等しい）と**仮定**しよう。この仮定のもとでは，古典派の二分法が成立し，実質変数である相対価格と実質為替レートは技術や選好などの「実質的」な要因のみに依存することになる。更に，ブレトンウッズ体制は崩壊の前後では「実質的」な要因の大きな変化は起きていないため，実質為替レートは大きく変化していない事になる。
# 
