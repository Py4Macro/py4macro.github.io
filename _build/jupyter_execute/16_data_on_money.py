...
# 古典派の二分法が成立している場合，相対価格は「実質的」な要因によって決定され，分母・分子の名目変数はマネーストックのような貨幣的な要因だけに依存することになる。この点を念頭にブレトンウッズ体制の崩壊の効果について考えよう。
...
# * 1973年2月14日に変動相場制に移行
# 
# この政策変更は名目為替レートに関しての政策であり，「実質的」な要因の変化ではない。純粋に貨幣的な政策変更である。まず，この金融政策変更の効果を思考実験として考えるために，全ての財・サービスの価格は十分に伸縮的であり，瞬時に価格は調整され市場は常に精算する（需要と供給は常に等しい）と**仮定**しよう。この仮定のもとでは，古典派の二分法が成立し実質変数である相対価格と実質為替レートは技術や選好などの「実質的」な要因のみに依存することになり，貨幣的な政策変更の影響は受けないはずである。更に，ブレトンウッズ体制は崩壊の前後では「実質的」な要因の大きな変化は起きていないため，実質為替レートは大きく変化していない事になる。
# 
...
# $$ 
# 
# 上の図を見ると$\dfrac{\dot{\varepsilon}}{\varepsilon}$は大きく動いていない。即ち，$\dfrac{\dot{P}_{\text{日}}}{P_{\text{日}}}-\dfrac{\dot{P}_{\text{日}}}{P_{\text{日}}}$の差は大きく動いていないということである。次に，1973年2月以降の変動相場制をの場合を考えてみよう。
# 
# $$
# \frac{\dot{\varepsilon}}{\varepsilon}=
# \frac{\dot{e}}{e}+
...
# $\dfrac{\dot{e}}{e}$は$\dfrac{\dot{P}_{\text{米}}}{P_{\text{米}}}-\dfrac{\dot{P}_{\text{日}}}{P_{\text{日}}}$にキャンセルされ，変動相場制の下でも$\dfrac{\dot{\varepsilon}}{\varepsilon}$は固定相場制と変わらない動きになるはずである。上の図はそうなっていないことを示している。即ち，$P_{\text{米}}$と$P_{\text{日}}$は十分に動かず，名目変数$e$の変動は実質変数$\varepsilon$の変化として姿を変えて現れたのである。その裏にあるのが**価格の粘着性**と考えられる。価格調整に時間がかかるため，$P_{\text{米}}$と$P_{\text{日}}$は大きく動かない。従って，$\dfrac{\dot{P}_{\text{米}}}{P_{\text{米}}}-\dfrac{\dot{P}_{\text{日}}}{P_{\text{日}}}$は$\dfrac{\dot{e}}{e}$をキャンセルする程大きくなかったという解釈である。
# 
# 為替制度の変更が価格調整スピードを瞬時に大きく変えるとは考えにくい。ブレトンウッズ体制崩壊前後での価格調整スピードは変わらないと考えるのが自然であろう。このように考えると，ブレトンウッズ体制の崩壊という歴史的な「実験」は，金融政策は実質変数に影響を与えることを示唆している。一方，前章のRBCモデルは貨幣の中立性が成立（仮定）されており，マクロ経済の短期的な動きを十分に捉えているとは言い難い。その点を克服する理論が次章で考察するニューケインジアン・モデルである。
