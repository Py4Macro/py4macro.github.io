...
#     * 「軸」を描いているのが（書き方１）の２行目である。右辺では、キャンバス`fig`のメソッド`add_subplot()`を使いキャンバスに軸を描き、それを左辺の変数`ax`に割り当てている。実際に、（書き方１）の２行のコードを実行すると透明のキャンバス上に軸が表示される。
...
# この章で付け加えたいもう一つのプロット方法がある。それが`Pandas`の`DataFrame`と`Series`のメソッド`plot()`である。これを使うと簡単にプロットできる場合が多々ある。
