...
# If you come here without expecting Japanese, please click [Google translated version](https://translate.google.com/translate?sl=auto&tl=en&u=https://py4macro.github.io/4_Matplotlib.html) in English or the language of your choice.
...
# `Matplotlib`（「マットプロットリブ」と読む）はプロットのための代表的なパッケージであり、ここではその使い方を解説する。プロットには`Matplotlib`のモジュールである`pyplot`を使うことになる。慣例に沿って`plt`としてインポートする。
...
# ```
# 
# 以下では「書き方２：オブジェクト指向」に沿って説明する。その為にまず、書き方１を使い`Matplotlib`の描画の手順を説明する。特に、「キャンバス」と「軸」の違いを覚えて欲しい。
# 1. 図を描くためのキャンバスを用意する。
...
#     * 「軸」を描いているのが書き方１の２行目である。右辺では、キャンバス`fig`のメソッド`add_subplot()`を使いキャンバスに軸を描き、それを左辺の変数`ax`に割り当てている。実際に、書き方１の２行のコードを実行すると透明のキャンバス上に軸が表示される。
#     * 作成された軸`ax`にメソッド`plot()`などを使い直線・曲線・点など付け加えたり、タイトルなどの「飾り付け」をおこない作図することになる。
#     * １つのキャンバスには複数の軸を描くことが可能である。
# 
# この説明でわかることは、`Matplotlib`で表示される「図」とは透明のキャンバス上に描かれ装飾された「軸」ということである。以下の説明では、キャンバス上の「図」を「軸」（英語で "axis"）と呼ぶことにする。
# 
# さて書き方２に話を戻すと、既に察している読者もいるかも知れないが、１行目は書き方１の最初の２行を１行に省略したものである。右辺の`plt.subplots()`は透明のキャンバスと軸の２つをタプルとして返す。最初の要素が「キャンバス」でありそれを左辺の`fig`に、２つ目の要素が軸であり`ax`に割り当てている。実際、１行目だけを実行すると軸が表示される。後は書き方１同様、`ax`のメソッド`plot()`などを使い軸を飾り付けして作図することになる。
# 
# ちなみに、書き方３は書き方２の３行を１行に省略した形になっており、本来は`Matlab`ユーザーの`Python`への移行を促す為に用意された書き方である。
