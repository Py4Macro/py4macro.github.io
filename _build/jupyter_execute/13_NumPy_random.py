...
# GDPなどのマクロ変数は変動し相関性や持続性が大きな特徴であるが，経済モデルでどのように捉えれば良いのだろうか。IS-MP-PCモデルのインパルス反応分析でもマクロ変数の変動や持続性を捉える動きはあるが，それがどの程度データと合致しているのか不明である。このような背景のもと本章の目的は次の2つとなる。
# * マクロ経済分析におけるランダム変数の役割について考察する。
# * `Python`でどのようにランダム変数を扱うことができるかを説明する。
# 
# では，なぜマクロ経済分析においてランダム変数が重要なツールとなっているのだろうか。この点を説明するために，まず`py4macro`に含まれるデータセット`jpn-q`を使いGDPの変動をプロットする。
...
# 
# １の考えによると，差分方程式$x_{t+1}=f(x_t)$のように来期のGDPは今期のGDPに依存している。マクロ変数の持続性を捉えるには必要不可欠な特徴と言えるだろう。しかし，今期のGDP（$x_t$）が与えられると来期のGDP（$x_{t+1}$）は自ずと決定さることになり，来期のGDPの予測は簡単なものとなる。もしそうであれば政策運営は非常に簡単であろうが，現実はそうではない。そういう意味では，１は現実を十分に捉えることができていない。一方，２の考えはランダム変数の実現値の連続としてGDPが観測され，今期のGDPが与えられても来期のGDPにはランダムな要素があるため，予測が難しいという特徴がある。この特徴こそが，マクロ変数の性質を捉えるには必要な要素であり，上のプロットに現れていると考えられる。主流のマクロ経済学では，消費者や企業の行動はフォーワード・ルッキングであり（将来を見据えた最適化行動であり），且つ経済全体ではランダムな（不確実な）要素が重要な役割を果たしているという考え基づいている。そのような分析枠組みの中で，景気循環の「真のメカニズム」を解明することが目的となっている。
# 
...
# 直感的にマクロ変数の特徴である持続性とは，前期の値が今期の値にどれだけ影響を持っているかを示す。以下では、同じホワイト・ノイズ$\varepsilon_t$を使い$\rho$の値を変えてpersistenceの違いを考えてみる。そのための関数を作成しよう。
