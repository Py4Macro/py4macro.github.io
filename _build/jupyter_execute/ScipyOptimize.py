...
# ＜`bisection`方の考え方＞
# * 中間値の定理に基づく。
# * `f(a)`と`f(b)`の符号が異なり`f(x)`が連続関数である場合、`a`と`b`の間には`f(x)`がゼロになる`x`の値が必ず存在する。
# * 上の図の`c=(a+b)/2`は`a`と`b`の中間値であり、次の３つの可能性がある。
#     * `f(c)=0`
#     * `f(c)>0`
#     * `f(c)<0`
# * 上の図のように`f(c)`と`f(a)`が同じ符号であれば、同じ計算を`c`と`b`の間でおこなう。
# * 同様の計算プロセスを続けると`f(x)=0`を満たす`x`の値に十分に近い値を求めることができる。
# 
# ＜注意点＞
