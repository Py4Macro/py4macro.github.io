...
# 本章の目的は，回帰分析，差分方程式，ランダム変数を駆使し，景気循環データに基づき定量（数量）マクロ経済分析をおこなうことである。使うモデルは，中級レベルのマクロ経済学ではお馴染みの総需要・総供給モデル（ADASモデル）である。また，ここでの定量分析とは，ADASモデルの数量的な特徴を確認し，次の問いを検討することである。
# 
