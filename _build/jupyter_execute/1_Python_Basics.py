...
# If you come here without expecting Japanese, please click [Google translated version](https://translate.google.com/translate?sl=auto&tl=en&u=https://py4macro.github.io/1_Python_Basics.html) in English or the language of your choice.
...
# [オブジェクトと属性](object)の節で属性を確認する`dir()`関数について説明した。`dir()`は組み込み関数であるため，パッケージやモジュールをインポートしなくても使えるので便利である。しかし，通常使う必要がないダンダー`__`が付いている属性（例えば，`__init__`）まで表示される。それらを省いて，属性のリストを見やすく表示する`see`モジュールを紹介する。使い方は簡単で，まず次のコードでインポートする。
