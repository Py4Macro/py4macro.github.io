...
# ちなみに`bool()`はブール型に変換するのではなく、`()`の中のオブジェクト（以下の説明を参照）の truth value （`True`か`False`のブール値）と呼ばれる値を返す。あるオブジェクトが「空」かどうかを確かめるの使われる。
...
# 上の例を見ると`and`と`or`は`True`もしくは`False`を返しているような印象を持つかも知れないが，実はそうではない。ある条件に従って，`and`は左の変数もしくは右の変数を返している。`or`も同様である。次の例では，そのルールに基づいて値を返している。
...
# 興味がある人は[これらのサイト](https://www.google.co.jp/search?q=%E3%83%96%E3%83%BC%E3%83%AB%E6%BC%94%E7%AE%97+python)を参考にしてみよう。
