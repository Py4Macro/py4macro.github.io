...
# * $a=-1$の場合
#     * $x_t$は振動する。
# * $a>1$の場合$x_t$は正の無限大に発散する。
# * $a<-1$の場合$x_t$は振動し発散する。
# * $|a|<1$の場合$x_t$は収束（$-1<a<0$の場合は振動）：
...
# ```{admonition} コードの説明
# 1. `x`はアップデート用の変数であり、初期値を1とする。
# 2. 初期値が入ったリストであり、このリストに計算結果を追加する。
# 3. 右辺の`x`は(1)にある`x`の値を使い、計算結果を左辺の`x`に割り当てる。この時点で(1)の`x`がアップデートされる。
# 4. 計算結果を(2)のリストに追加する。
# ```
