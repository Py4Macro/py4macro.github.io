...
# 行はインデックス（番号）のままなので，列`year`を行ラベルに設定する。
# 
...
# メソッド`.reset_index()`を使うと，行のインデックスを0,1,2,..と振り直すことができる。`df`を使い説明する。
...
# ここでは行のインデックスが`year`として新たな列として追加されているが、`reset_index()`に引数`drop=True`を加えると，列`year`が自動的に削除される。
...
# ````{warning}
# `s`の場合、順番を表すインデックスで要素アクセスしようとするとエラーが発生する。これはインデックス・ラベルが整数になっている理由のようである。この場合は、インデックス・ラベルである`2000`や`2008`を使い次のようにするとエラーは発生しない。
# ```
# s[2000]
# ```
# ````

# 次にインデックス・ラベルを使った要素のアクセス方法を考える。この場合は辞書と同じ形になる。
